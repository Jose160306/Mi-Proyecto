...
function Layout() {
  ...
  // El menu de Administracion solo aparece si el usuario es admin
  const items = [
    { to: "/",             label: "Inicio",         icon: "🏠", end: true },
    { to: "/vehiculos",    label: "Vehículos",       icon: "🚗" },
    { to: "/espacios",     label: "Espacios",        icon: "🅿️" },
    { to: "/tickets",      label: "Tickets",         icon: "🎫" },
    ...(usuarioActual?.esAdmin ? [{ to: "/administrador", label: "Administración", icon: "⚙️" }] : []),
  ...
  {/* OVERLAY(cubrir) oscuro cuando el menu esta abierto en movil */}
  {menuAbierto && (
  <div
  onClick={cerrarMenu}
  style={{
  ...
  zIndex: 40
    ...
    />
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        {/* HEADER con boton hamburguesa (menu)*/}
        <header style={{
          height: 60,
          background: "white",
